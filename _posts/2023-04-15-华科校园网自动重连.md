...
这里我们需要做的就是把登陆界面的 content 给搞下来，首先进入校园网登陆界面：

![CleanShot-QzFqPQ29@2x]({{site.baseurl}}/assets/img/2023-04-15-华科校园网自动重连.assets/CleanShot-QzFqPQ29@2x.png)

然后按 F12 打开控制台

![CleanShot-I3xIFoBi@2x]({{site.baseurl}}/assets/img/2023-04-15-华科校园网自动重连.assets/CleanShot-I3xIFoBi@2x.png)

然后在 42 行这里设置 breakpoint 然后点击网页上的**连接 Login**就能得到 content 的内容，接下来只用把 content 的内容复制下来并保存到名为`content`的文件内和源码放在同一目录下就行了

_注：复制下来的 content 有双引号，需要手动去除_
